...
open OUT, ">$OrthoGroup.renamed.og" or die;
while(<OrthoGroup>){
      chomp;
      ...
      if(!/^\#/){
         ...
         if(exists $matingNames{$tem[1]}){
            my $newName = $matingNames{$tem[1]};
            print OUT "$tem[0]\t$newName\t$tem[2]\t$tem[3]\t$tem[4]\t$tem[5]\t$tem[6]\t$tem[7]\t$tem[8]\n";
        }
        else {
             print OUT "$_ does not exist\n";
        }
      }
}
close READ;
